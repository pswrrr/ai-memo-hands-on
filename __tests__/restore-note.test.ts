...
describe('restoreNote', () => {
  ...
  it('사용자가 인증되지 않은 경우 오류를 반환해야 한다', async () => {
    ...
    // 실행
    const result = await restoreNote('test-note-id');
  ...
  it('노트가 존재하지 않는 경우 오류를 반환해야 한다', async () => {
    ...
    // 실행
    const result = await restoreNote('non-existent-note-id');
  ...
  it('다른 사용자의 노트를 복구하려고 하면 오류를 반환해야 한다', async () => {
    // Mock 설정
    (createServerSupabase as jest.Mock).mockResolvedValue({
      auth: {
        getUser: jest.fn().mockResolvedValue({
    ...
    // 검증
    expect(result.success).toBe(false);
    expect(result.error).toBe('이 노트를 복구할 권한이 없습니다');
  });

  it('성공적으로 노트를 복구해야 한다', async () => {
    // Mock 설정
    (createServerSupabase as jest.Mock).mockResolvedValue({
      auth: {
        getUser: jest.fn().mockResolvedValue({
