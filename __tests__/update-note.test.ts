// __tests__/update-note.test.ts
// updateNote 서버 액션 테스트 - 노트 수정 기능 검증
// 사용자 인증, 권한 검증, 데이터 유효성 검증 등의 로직을 테스트
...
describe('updateNote', () => {
  ...
  it('존재하지 않는 노트는 에러를 반환해야 함', async () => {
    ...
    const result = await updateNote('non-existent-note', 'New Title', 'New Content');
  ...
  it('자신의 노트는 성공적으로 수정되어야 함', async () => {
    const mockUser = { id: 'user-123', email: 'test@example.com' };
    const mockNote = {
      id: 'note-123',
  ...
  it('데이터베이스 오류 시 에러를 반환해야 함', async () => {
    ...
    expect(result.success).toBe(false);
    expect(result.error).toBe('노트 수정에 실패했습니다');
  });
});
