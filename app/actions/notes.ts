...
export async function deleteNote(formData: FormData) {
  try {
    ...
    // 유효성 검증
    const validatedData = deleteNoteSchema.parse(rawData);

    // 노트 소유권 확인 및 삭제
    const result = await db
      .delete(notes)
      .where(eq(notes.id, validatedData.id) && eq(notes.userId, user.id))
      .returning();

    if (result.length === 0) {
      return {
        success: false,
        error: NOTE_ERROR_MESSAGES.NOT_FOUND,
      };
    }
    ...
    return {
      success: false,
      error: NOTE_ERROR_MESSAGES.DELETE_FAILED,
    };
  }
}
